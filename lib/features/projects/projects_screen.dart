    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(r.count == 0 ? 'Todo al día.' : '${r.count} proyecto(s) sincronizado(s).')),
        );
      }
    } on ApiError catch (e) {
    ...
    final ok = await showModalBottomSheet<bool>(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          SizedBox(width: double.infinity, child: FilledButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Crear'))),
        ]),
      ),
    );
    if (ok == true && name.text.trim().isNotEmpty) {
      final p = await api.post('/projects/', {'name': name.text.trim(), 'description': desc.text.trim()});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        backgroundColor: kPrimary, onPressed: _create, icon: const Icon(Icons.add), label: const Text('Nuevo'),
      ),
      body: _loading
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  Row(children: [
                    const Expanded(child: Text('Proyectos', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800))),
                    IconButton(
                      tooltip: 'Sincronizar',
                  ...
                  if (_projects.isEmpty)
                    const Padding(padding: EdgeInsets.all(16), child: Text('No hay proyectos.', style: TextStyle(color: kMuted))),
                  ..._projects.map((p) => Card(
                        child: ListTile(
                          ...
                          title: Text(p.name),
                          subtitle: Text(p.ownerEmail, style: const TextStyle(fontSize: 12)),
