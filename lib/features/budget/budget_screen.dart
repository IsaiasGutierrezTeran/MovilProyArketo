...
class _BudgetScreenState extends State<BudgetScreen> {
  ...
  Future<void> _create() async {
    if (_materials.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No hay materiales en el catálogo.')));
      return;
    }
    final created = await showModalBottomSheet<bool>(
      context: context, isScrollControlled: true, backgroundColor: kSurface,
      builder: (_) => _NewBudgetSheet(projectId: widget.projectId, materials: _materials, api: _api),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        backgroundColor: kPrimary, onPressed: _create,
                ...
                children: [
                  ...
                  ..._budgets.map((b) => Card(child: Padding(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Row(children: [
                            Expanded(child: Text('Total: ${b.total} ${b.currency}',
...
class _NewBudgetSheet extends StatefulWidget {
  ...
  final Api api;
  const _NewBudgetSheet({required this.projectId, required this.materials, required this.api});
  @override
...
class _NewBudgetSheetState extends State<_NewBudgetSheet> {
  ...
  Future<void> _save() async {
    final items = _rows
        .where((r) => r.material != null && (double.tryParse(r.qty.text) ?? 0) > 0)
        .map((r) => {'material': r.material, 'quantity': r.qty.text})
        .toList();
    if (items.isEmpty) { setState(() => _error = 'Añade al menos un material.'); return; }
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Nuevo presupuesto', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
          ...
          ..._rows.asMap().entries.map((e) {
            ...
            return Padding(
              ...
              child: Row(children: [
                ...
                Expanded(flex: 1, child: TextField(controller: r.qty, keyboardType: TextInputType.number,
                    decoration: const InputDecoration(labelText: 'Cant.'))),
                IconButton(icon: const Icon(Icons.close, size: 18),
                    onPressed: _rows.length > 1 ? () => setState(() => _rows.removeAt(e.key)) : null),
              ]),
            );
          }),
          TextButton.icon(onPressed: () => setState(() => _rows.add(_Row())),
              icon: const Icon(Icons.add, size: 18), label: const Text('Añadir material')),
          const SizedBox(height: 8),
          Row(children: [
            Expanded(child: TextField(controller: _people, keyboardType: TextInputType.number,
                decoration: const InputDecoration(labelText: 'Mano de obra (personas)'))),
            const SizedBox(width: 8),
            Expanded(child: TextField(controller: _laborCost, keyboardType: TextInputType.number,
                decoration: const InputDecoration(labelText: 'Costo mano de obra'))),
          ]),
